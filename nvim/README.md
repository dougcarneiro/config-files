...
```sh
git clone git@github.com:dougcarneiro/my-nvim-config.git "${XDG_CONFIG_HOME:-$HOME/.config}"/nvim
```
...
sudo apt update
sudo apt install fd-find make gcc ripgrep unzip git neovim
```
</details>
...
```
sudo apt update
sudo apt install fd-find make gcc ripgrep unzip git
